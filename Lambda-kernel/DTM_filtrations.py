...
def DTM(X,query_pts,m):
    ...
    NN_Dist = NN_Dist.reshape((query_pts.shape[0],k))
    DTM_result = np.sqrt(np.sum(NN_Dist*NN_Dist,axis=1) / k)
    return(DTM_result)

def DTM_DM(dm,query_pts,m):
    '''
    Compute the values of the DTM (with exponent p=2) of the empirical measure of a distance matrix dm
    Require sklearn.neighbors.KDTree to search nearest neighbors
    ...
    DTM_values = DTM(X, Q, 0.3)
    '''
    #assert np.all(dm==query_pts)
    N_tot = dm.shape[0]     
    k = math.floor(m*N_tot)+1   # number of neighbors
    #kdt = KDTree(dm, leaf_size=30, metric='precomputed')
    #NN_Dist, NN = kdt.query(query_pts, k, return_distance=True)  
    nbrs = NearestNeighbors(n_neighbors=k,metric='precomputed').fit(dm)
    NN_Dist, NN = nbrs.kneighbors(query_pts)
    NN_Dist = NN_Dist.reshape((query_pts.shape[0],k))
    DTM_result = np.sqrt(np.sum(NN_Dist*NN_Dist,axis=1) / k)
    return(DTM_result)
...
def WeightedRipsFiltration_DM(dm, F, p, dimension_max =2, filtration_max = np.inf):
    '''
    Compute the weighted Rips filtration of a distance matrix, weighted with the 
    values F, and with parameter p
    ...
    '''
    N_tot = dm.shape[0]     
    distances = dm         # compute the pairwise distances
    st = gudhi.SimplexTree()                    # create an empty simplex tree
...
def DTMFiltration_DM(dm, m, p, dimension_max =2, filtration_max = np.inf):
    '''
    Compute the DTM-filtration of a distance matrix, with parameters m and p
    ...
    st: a gudhi.SimplexTree 
    '''
    DTM_values = DTM_DM(dm,dm,m)
    st = WeightedRipsFiltration_DM(dm, DTM_values, p, dimension_max, filtration_max)
    return st
